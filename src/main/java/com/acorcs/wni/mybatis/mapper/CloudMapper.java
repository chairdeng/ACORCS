...
public interface CloudMapper extends WniEntityMapper<Cloud> {
    ...
    @Select("SELECT id,notice_id,header,cloud_distribution,cloud_type,altitudes,airframe_icing,extended_degree,AsBinary(geographic) as geographic,original FROM wni_cloud WHERE notice_id=#{noticeId}")
    @Results({
